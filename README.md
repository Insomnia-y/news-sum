...
$$
L_{xent} = -\sum_{j=1}^l \sum_s p_{true}(s|D, S_{<j}^r) \log p_{g_\theta} (s|D, S_{<j}^r; \theta)
$$
...
$$
p_{true}(s|D, S_{<j}^r) = \begin{cases}
1 - \beta, &s=s_j^r \\
\frac{\beta}{N-1}, &s \not= s_j^r
\end{cases}
$$
...
$$
p_{g_\theta} (s_t|D, S_{<j}; \theta)
$$


上式与交叉熵函数最大的不同模型在预测下一个 token 时，用的是之前预测的序列 $S_{<j}$ 代替参考序列 $S_{<j}^{r}$，这就导致出现 exposure bias 问题：即使模型 $g$ 训练拟合的很好，一但在预测过程中 $S_{<j}$ 出现偏移，跟 $S_{<j}^{r}$ 不一致，模型预测的性能就会衰退、下降的问题。
...
f(S) = \frac{\sum_{t=1}^l \log p_{g_\theta} (s_t|D, S_{<j}; \theta)}{|S|^\alpha}
$$


其中 $f(S_i)$ 为长度正则化的解码概率， $L_{ctr}$ 为对比损失，$i,j$ 为候选摘要排序后的索引，越小意味越好；即当 $j>i$ 时，$\text{ROUGE}(S_i,S^{r}) > \text{ROUGE}(S_j,S^{r})$。其中 $\lambda_{ij} = (j-i) * \lambda$，为排名次数的边际调整参数。
